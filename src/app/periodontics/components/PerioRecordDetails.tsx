...
export default function PerioRecordDetails({ record, onEdit, onDelete, onBack }: PerioRecordDetailsProps) {
  return (
    <div className="space-y-6">
      ...
      <ParameterEntriesSection record={record} />
      <DemographicsSection record={record} onEdit={onEdit} />
      <RecordInformationCard record={record} />
    </div>
  );
}

function DemographicsSection({ record, onEdit }: { record: PerioRecord; onEdit: () => void; }) {
...
function ParameterEntriesSection({ record }: { record: PerioRecord; }) {
...
function RecordInformationCard({ record }: { record: PerioRecord }) {
